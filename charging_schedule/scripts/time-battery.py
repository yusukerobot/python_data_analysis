...
plt.rcParams['font.size'] = 55  # フォントサイズ設定

# グラフの寸法設定（cmからインチに変換）
width_cm = 16.5  # 幅の設定（cm）
height_cm = width_cm / 1.6  # 高さを幅の1.6倍に設定
width_inch = width_cm / 2.54  # cmからインチに変換
height_inch = height_cm * 1.3 / 2.54  # cmからインチに変換

# グラフの作成
fig, ax = plt.subplots(figsize=(width_inch, height_inch))
ax.plot(time, battery, label='Battery Level', color='blue', linewidth=5)

# 軸ラベルとタイトルの設定
ax.set_xlabel('Time [min]')
ax.set_ylabel(r'Battery Level [%]')

plt.xticks(np.arange(0, 120, step=25))
