...
def calculate_stats_and_distances(input_path):
    ...
    ax1.bar(generations_list, f1_means, yerr=f1_stds, label='f1', alpha=0.6, color='blue', capsize=5)
    ax1.bar(generations_list, f2_means, yerr=f2_stds, label='f2', alpha=0.6, color='green', capsize=5)

    ax1.set_xlabel('$\eta$', color='black')
    ax1.set_ylabel('Objective function Value', color='black')
